			...
			<c:when test="<%= repository == null %>">
				<aui:select id="repositoryTypes" label="repository-type" name="className">
					...
					for (String dlRepositoryImpl : RepositoryFactoryUtil.getRepositoryClassNames()) {
						if (StringUtil.equalsIgnoreCase(dlRepositoryImpl, "com.bvakili.portlet.integration.box.repository.BoxRESTRepository")) {
						%>

							<aui:option label="<%= ResourceActionsUtil.getModelResource(locale, dlRepositoryImpl) %>" onClick="boxComConfirmation()" value="<%= dlRepositoryImpl %>" />

						<%
						} else {
					%>

							<aui:option label="<%= ResourceActionsUtil.getModelResource(locale, dlRepositoryImpl) %>" onClick="" value="<%= dlRepositoryImpl %>" />
